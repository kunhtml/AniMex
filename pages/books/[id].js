...
export default function BookPage() {
  ...
  useEffect(() => {
    ...
    fetchBooks().then(data => {
      if (mounted && data) {
        ...
        setRecommendedBooks(shuffled.slice(0, 8)); // Get 8 random books
      }
    });
  ...
  return (
    <>
      <Head>
        <title>{book.title} — Hang Truyện</title>
      </Head>
      <Header />
      <main className="container">
        <div className="book-detail-card">
          ...
          <div className="book-info">
            <h2>{book.title}</h2>
            <p className="book-meta"><em>{book.author}</em> · {book.genre} · {book.grade}</p>
        ...
        <section className="content-preview" style={{marginTop:'2rem'}}>
          <h3>Nội dung (preview)</h3>
          <div>
            {Array.isArray(book.content) ? book.content.slice(0,5).map((chapter, i) => (
              <p key={i}>{chapter}</p>
            )) : <p>{book.content}</p>}
          </div>
        </section>
